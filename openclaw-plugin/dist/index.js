...
const configSchema = Type.Object({
    apiBaseUrl: Type.Optional(Type.String({
        description: "Python FastAPI base URL, for example http://127.0.0.1:8000",
    })),
...
const actionParams = Type.Object({
    authToken: Type.String({
        description: "Required. Auth token returned only by verify_bank_pin after pin_correct is true. Never show this token to the customer.",
    }),
    userStatement: Type.String({
        description: "Required. The customer's original problem statement, not the PIN message.",
    }),
    ...
    cardName: Type.Optional(Type.String({
        description: "Affected card display name, for example Visa Platinum Credit Card ending 4382.",
    })),
    cardType: Type.Optional(Type.String({
        description: "Affected card type or network. Accepted examples: credit, debit, unknown, VISA, RUPAY, MASTERCARD.",
    })),
    cardNetwork: Type.Optional(Type.String({
    ...
    transactionLocation: Type.Optional(Type.String({
        description: "Transaction/location mentioned by the customer, if any.",
    })),
    homeCity: Type.Optional(Type.String({
        description: "Customer home city returned by verification API.",
    })),
});
function pinRequiredBlock(input, reason) {
    return {
        caseId: "PIN-REQUIRED",
        selectedTool: "workflow_guard",
        status: "blocked",
        customerMessage: "PIN verification has not been completed, so no card safeguard action was applied yet.",
        detailedSteps: [
...
function verifyActionAllowed(input, config) {
...
export default defineToolPlugin({
    ...
    description: "Strict bank-card protection workflow tools. Correct order: verify_bank_customer, ask PIN, verify_bank_pin, then exactly one safeguard tool.",
    configSchema,
    tools: (tool) => [
        tool({
            name: "verify_bank_customer",
            label: "1. Verify Bank Customer",
            description: "FIRST TOOL ONLY. Use this before asking for PIN and before any card protection tool. Input: registered phone number and original user concern.",
            parameters: Type.Object({
                phone: Type.String({
                    description: "Customer phone number with country code, for example +919876543210.",
                }),
                userStatement: Type.String({
                    description: "The user's original concern about fraud, theft, lost card, or PIN/card exposure.",
                }),
            }),
            async execute({ phone, userStatement }, config, context) {
                ...
                return {
                    ...
                    result,
                    next_agent_instruction: "If result.account_exists is true, stop using tools and ask the user only for their 6 digit BANK PIN. Do not call any safeguard tool yet. Save result.statement_analysis.suggested_primary_tool for fast routing after PIN verification.",
                };
            },
        }),
        tool({
            ...
            description: "SECOND TOOL ONLY. Use only after verify_bank_customer succeeds and the user provides a 6 digit BANK PIN. Never show the PIN or auth_token to the user.",
            parameters: Type.Object({
                phone: Type.String({
                    description: "Verified customer phone number with country code from verify_bank_customer.",
                }),
                name: Type.String({
            ...
            async execute({ phone, name, pin }, config, context) {
                context.signal?.throwIfAborted();
                ...
                return {
                    ...
                    result,
                    next_agent_instruction: "If result.pin_correct is true, immediately call exactly one safeguard tool using the earlier verify_bank_customer result.statement_analysis.suggested_primary_tool. Do not compare all tools slowly. Do not ask another question. Never reveal result.auth_token.",
                };
            },
        }),
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use after verify_bank_pin returns pin_correct true. Select this for unauthorized or suspicious CREDIT CARD spend, charge, transaction, purchase, amount, merchant, or location.",
        ...
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use after verify_bank_pin returns pin_correct true. Select this for stolen card, wallet stolen, robbed, or physical card taken.",
            parameters: actionParams,
            async execute(input, config, context) {
                context.signal?.throwIfAborted();
                const blocked = verifyActionAllowed(input, config);
        ...
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use after verify_bank_pin returns pin_correct true. Select this for exposed PIN, visible PIN, leaked PIN, shared PIN, OTP, CVV, password, online photo, social media post, or card details exposure.",
            parameters: actionParams,
            async execute(input, config, context) {
                context.signal?.throwIfAborted();
                const blocked = verifyActionAllowed(input, config);
        ...
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use after verify_bank_pin returns pin_correct true. Select this for lost, missing, misplaced, or cannot-find card when there is no fraud transaction, theft, or PIN/card exposure.",
            parameters: actionParams,
            async execute(input, config, context) {
                context.signal?.throwIfAborted();
                const blocked = verifyActionAllowed(input, config);
