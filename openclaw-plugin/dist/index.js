...
export default defineToolPlugin({
    ...
    description: "Strict bank-card protection workflow tools. The correct order is verify_bank_customer, ask PIN, verify_bank_pin, then exactly one safeguard tool.",
    configSchema,
    tools: (tool) => [
        tool({
            name: "verify_bank_customer",
            label: "1. Verify Bank Customer",
            description: "FIRST TOOL ONLY. Use this before asking for PIN and before any card protection tool. Input is the registered phone number and the user's original statement/concern. If phone is missing, ask the user for phone instead of using any tool.",
            ...
            async execute({ phone, userStatement }, config, context) {
                ...
                return {
                    ...
                    result,
                    next_agent_instruction: "If result.account_exists is true, stop using tools and ask the user only for their 6 digit BANK PIN. Do not analyze or call safeguard tools yet. If false, do not ask PIN and route to manual support.",
                };
            },
        }),
        tool({
            ...
            description: "SECOND TOOL ONLY. Use only after verify_bank_customer returned account_exists true and the user has provided a 6 digit BANK PIN. Never use this before customer verification. Never show the PIN or auth token to the user.",
            parameters: Type.Object({
            ...
            async execute({ phone, name, pin }, config, context) {
                ...
                return {
                    ...
                    result,
                    next_agent_instruction: "If result.pin_correct is true, analyze the original user statement and choose exactly one safeguard tool from result.allowed_resolution_tools. Priority: compromised PIN/card details exposure > credit card fraud transaction > physical card theft > lost card. Never reveal result.auth_token.",
                };
            },
        }),
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use only after verify_bank_pin returned pin_correct true and authToken is available. Select this when the original statement describes unauthorized/suspicious CREDIT CARD spend, charge, transaction, purchase, amount, merchant, or location. Do not use for stolen/lost card unless an unauthorized credit-card transaction is the main issue.",
        ...
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use only after verify_bank_pin returned pin_correct true and authToken is available. Select this when the original statement describes physical card theft, wallet stolen, robbed, card taken, or someone stole the card. Do not use for lost/misplaced cards unless theft is stated.",
            parameters: actionParams,
            async execute(input, config, context) {
                context.signal?.throwIfAborted();
                const blocked = verifyActionAllowed(input, config);
        ...
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use only after verify_bank_pin returned pin_correct true and authToken is available. Select this for PIN, OTP, CVV, password, or card details exposure; online photo/picture/social media post; visible PIN; or card/PIN seen by others. This has higher priority than fraud/theft/lost if secret exposure is mentioned.",
            parameters: actionParams,
            async execute(input, config, context) {
                context.signal?.throwIfAborted();
                const blocked = verifyActionAllowed(input, config);
        ...
        tool({
            ...
            description: "POST-PIN TOOL ONLY. Use only after verify_bank_pin returned pin_correct true and authToken is available. Select this when the original statement says the credit/debit card is lost, missing, misplaced, or cannot be found, and there is no clear theft, fraud transaction, or PIN/card details exposure.",
            parameters: actionParams,
            async execute(input, config, context) {
                context.signal?.throwIfAborted();
                const blocked = verifyActionAllowed(input, config);
