import type { ActionInput, ActionResult } from "./types.js";

function caseId(prefix: string): string {
  const stamp = new Date().toISOString().replace(/[-:TZ.]/g, "").slice(0, 14);
  const random = Math.random().toString(36).slice(2, 8).toUpperCase();
  return `${prefix}-${stamp}-${random}`;
}

function safeCardName(cardName?: string, cardType?: string): string {
  if (cardName && cardName.trim().length > 0) return cardName.trim();
  if (cardType === "credit") return "the selected credit card";
  if (cardType === "debit") return "the selected debit card";
  return "the affected card";
}

function locationLine(input: ActionInput): string {
  if (!input.transactionLocation) return "No transaction location was provided by the user statement.";
  if (!input.homeCity) return `Transaction/location mentioned: ${input.transactionLocation}.`;
...
function amountLine(input: ActionInput): string {
  if (!input.amount) return "No exact amount was provided by the user statement.";
  return `Amount mentioned by user: ${input.currency ?? "UNKNOWN"} ${input.amount}.`;
}

export function creditCardFraud(input: ActionInput): ActionResult {
...
export function anyCardTheft(input: ActionInput): ActionResult {
...
export function compromisedPin(input: ActionInput): ActionResult {
  const card = safeCardName(input.cardName, input.cardType);
  return {
...
export function anyCardLost(input: ActionInput): ActionResult {
  const card = safeCardName(input.cardName, input.cardType);
  return {
