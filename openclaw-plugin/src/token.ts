...
function sign(input: string, sharedSecret: string): Buffer {
  return crypto.createHmac("sha256", sharedSecret).update(input).digest();
}

export function verifyBankAuthToken(authToken: string, sharedSecret: string, expectedPhone: string, expectedName: string): AuthClaims {
  ...
  const claims = JSON.parse(base64UrlDecode(payloadPart).toString("utf-8")) as AuthClaims;
